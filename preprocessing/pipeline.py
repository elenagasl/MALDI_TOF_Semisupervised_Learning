"""
MALDI-TOF Spectrum Preprocessing Pipeline
==========================================

Provides composable, callable preprocessing transforms for MALDI-TOF mass
spectra, following the sequential pipeline used in this study:

    VarStabilizer → Smoother → BaselineCorrecter → Trimmer
        → Binner → StdThresholder → LogScaler

Each transform accepts a ``SpectrumObject`` and returns a new
``SpectrumObject`` with the transformation applied, leaving the input
unchanged.
...
::
    ...
    from preprocessing.pipeline import (
        SequentialPreprocessor, VarStabilizer, Smoother,
        BaselineCorrecter, Trimmer, Binner, StdThresholder, LogScaler,
    )
    ...
    pipeline = SequentialPreprocessor(
        ...
        StdThresholder(factor=2.0),
        LogScaler(base=10),
    )

    processed = pipeline(raw_spectrum)

The result is a fixed-length intensity vector of 6 000 values (one per
3 Da bin over 2 000–20 000 Da), suitable for direct input into any of
the neural models defined in this repository.
"""
