...
Model:
    Input = (MALDI spectrum, species ID, antibiotic ID)
...
Protocol:
    For each run:
        1. Select species common to source and Göttingen.
        2. Align antibiotics common to both datasets.
        3. Train global recommender on MARISMA + DRIAMS.
        4. Split Göttingen by species into:
            - fine-tuning subset
            - OOD test subset
        5. Evaluate source-trained model on Göttingen test.
        6. Fine-tune model on Göttingen fine-tuning subset.
        7. Re-evaluate on same Göttingen test subset.

Metrics:
    - global micro AUC before/after fine-tuning
    - global macro antibiotic AUC before/after fine-tuning
    - per-species micro/macro AUC before/after fine-tuning
    - per-antibiotic AUC before/after fine-tuning

Recommended command:

nohup python run_ood_gottingen_global_recommender_gpu.py \
  ...
  --output-dir "/export/usuarios01/egarroyo/MALDI_for_AMR_prediction/src/benchmark study/benchmark_full/benchmark_outputs/ood_gottingen_global_recommender" \
  --device cuda \
  --n-runs 5 \
  --finetune-frac 0.2 \
  --source-epochs 300 \
  ...
  --batch-size 128 \
  --val-batch-size 2048 \
  > ood_gottingen_global_recommender.log 2>&1 &
"""
