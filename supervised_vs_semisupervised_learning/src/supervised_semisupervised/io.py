...
def load_combined_pickle(path: str | Path) -> PicklePayload:
    """Load the combined DRIAMS/MARISMa pickle.

    Expected payload keys:
        data, label, amr, antibiotics
    """
    ...
    if amr.ndim != 2:
        raise ValueError(f"Expected amr to be 2D, got shape {amr.shape}")
    if X.shape[0] != species.shape[0] or X.shape[0] != amr.shape[0]:
        raise ValueError(
            "Inconsistent sample counts: "
            f"X={X.shape[0]}, species={species.shape[0]}, amr={amr.shape[0]}"
        )
    if amr.shape[1] != len(antibiotics):
        raise ValueError(
            f"AMR columns ({amr.shape[1]}) do not match antibiotics ({len(antibiotics)})"
        )
