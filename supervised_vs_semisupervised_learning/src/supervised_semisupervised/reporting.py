...
def _load_prediction_block(path: Path) -> dict[str, object]:
    ...
    if "species_name" in data:
        ...
        species_name = path.stem.rsplit("_fold_", maxsplit=1)[0].replace("_", " ")
    return {
        "path": path,
        "model": path.parent.name,
        "species": species_name,
        "fold": int(data["fold"].item()) if "fold" in data else None,
...
def write_final_reports(output_dir: Path, antibiotics: list[str]) -> None:
    """Create final CSV reports from saved fold-level predictions.

    Generated files:
        - global_metrics_by_model.csv
        - species_metrics_by_model.csv
        - auc_by_species_antibiotic/<model>.csv
    """
    ...
    models = sorted({str(block["model"]) for block in blocks})
    for model in models:
        ...
        species_values = sorted({str(block["species"]) for block in model_blocks})
        for species in species_values:
            ...
            for antibiotic_idx in antibiotic_indices:
                ...
                row = {
                    "model": model,
                    "species": species,
                    "antibiotic_idx": int(antibiotic_idx),
                    "antibiotic": antibiotics[int(antibiotic_idx)],
                    "auc": auc,
