"""
MARISMa Dataset Manager
=========================

Provides data loading and indexing for the MARISMa (MALDI-TOF AMR at
Hospital Ramón y Cajal, Madrid) clinical dataset.
...
AMR labels are provided separately as an anonymised pickle file
(``MARISMa_anonymized.pkl``) which is joined to the spectra by study ID.

Classes
-------
MARISMaManager
    Traverses the hierarchical folder structure to index all available
    spectra. Supports saving/loading to pickle.

    ``spectra_dict[year][genus][species][study_id]`` → list of binary paths

MARISMa(Dataset)
    PyTorch Dataset that loads, preprocesses and returns
    (SpectrumObject, label, meta) tuples on demand.
"""
