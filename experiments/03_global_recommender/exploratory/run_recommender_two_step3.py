# ============================================================
# AUC-FILTERED CALIBRATED TWO-STAGE CONTEXT EXPERIMENTS
#
# Experiments:
# 1. auc_filter_threshold_10_90
# 2. auc_filter_topk_3
# 3. auc_filter_topk_5
# 4. auc_filter_corr_pos_topk_3
# 5. auc_filter_corr_pos_topk_5
# 6. auc_filter_corr_abs_topk_3
# 7. auc_filter_corr_abs_topk_5
#
# Logic:
#   1) Is this contextual antibiotic well predicted?
#      -> Stage 1 validation AUC >= MIN_CONTEXT_VAL_AUC
#
#   2) Am I confident for this sample?
#      -> threshold or |p - 0.5|
#
#   3) Is this antibiotic related to the target?
#      -> train phi correlation with the target
# ============================================================
...
OUTPUT_DIR = "auc_filtered_context_results"
...
# Key new filter:
# Only antibiotics with Stage 1 validation AUC >= this value
# can be used as context.
MIN_CONTEXT_VAL_AUC = 0.80

# Minimum observations to compute antibiotic-level AUC in validation.
MIN_AUC_OBS = 20

# Minimum paired observations to compute train correlations.
MIN_CORR_PAIR = 30

# Minimum observations to fit Platt calibration per antibiotic.
...
EXPERIMENTS = [
    {
        "name": "auc_filter_threshold_10_90",
    ...
    {
        "name": "auc_filter_corr_pos_topk_3",
    ...
    {
        "name": "auc_filter_corr_pos_topk_5",
        "context_mode": "corr_topk",
        "low_threshold": None,
        "high_threshold": None,
        "top_k": 5,
    ...
    {
        "name": "auc_filter_corr_abs_topk_3",
        "context_mode": "corr_topk",
        "low_threshold": None,
        "high_threshold": None,
        "top_k": 3,
    ...
    {
        "name": "auc_filter_corr_abs_topk_5",
        "context_mode": "corr_topk",
        "low_threshold": None,
        "high_threshold": None,
        "top_k": 5,
...
class Stage1Recommender(pl.LightningModule):
    """
    Stage 1:
        MALDI embedding + antibiotic embedding -> AMR logit per antibiotic.
    """
...
class Stage2ContextPredictor(pl.LightningModule):
    """
    Stage 2:
        MALDI embedding + antibiotic embedding + filtered context
        -> final AMR prediction.

    The context is filtered by:
        1. reliable antibiotic mask from validation AUC
        2. confidence per sample
        3. correlation with target if corr_topk mode
    """
    ...
    def __init__(
        ...
        self.save_hyperparameters(
            ignore=["corr_matrix", "reliable_context_mask"]
        )
        ...
        input_dim = (
            maldi_emb_dim +
            drug_emb_dim +
            num_items +     # context values
            num_items       # context mask
        )
        ...
        if reliable_context_mask is None:
            reliable_context_mask = np.ones(num_items, dtype=np.float32)

        corr_tensor = torch.tensor(corr_matrix).float()
        reliable_tensor = torch.tensor(reliable_context_mask).float()

        self.register_buffer("corr_matrix", corr_tensor)
        self.register_buffer("reliable_context_mask", reliable_tensor)
    ...
    def expand_context_per_target(self, values_base, mask_base):
        ...
        # Exclude target antibiotic from its own context.
    ...
    def build_threshold_context(self, context_probs):
        ...
        # AUC filter:
        # only reliable antibiotics can enter context.
        reliable = self.reliable_context_mask.view(1, -1)

        values_base = values_base * reliable
        mask_base = mask_base * reliable
    ...
    def build_topk_context(self, context_probs):
        ...
        # Unreliable antibiotics cannot be selected.
        score = confidence * reliable

        if k is None or k <= 0:
            mask_base = torch.zeros_like(context_probs)
            values_base = values_base * mask_base

            return self.expand_context_per_target(values_base, mask_base)

        k = min(k, num_items)

        # If there are fewer reliable antibiotics than k, topk will still
        # return some zero-score positions. We remove them afterwards.
        _, idx = torch.topk(score, k=k, dim=1)

        mask_base = torch.zeros_like(context_probs)
        mask_base.scatter_(1, idx, 1.0)

        # Remove zero-score selections.
        mask_base = mask_base * (score > 0).float()
    ...
    def build_corr_topk_context(self, context_probs):
        ...
        # Exclude self-context in correlation matrix.
        eye = torch.eye(num_items, device=device)
        relevance = relevance * (1.0 - eye)

        # AUC reliability filter.
        # reliable_context_mask is over context antibiotic dimension.
        reliable = self.reliable_context_mask.view(1, 1, -1)
        ...
        _, idx = torch.topk(score, k=k, dim=2)
...
def compute_antibiotic_auc_table(y_true, preds, antibiotic_names):
    rows = []
    ...
    for j in range(num_items):
        ...
        n_pos = int(np.sum(col[valid] == 1)) if n_obs > 0 else 0
        n_neg = int(np.sum(col[valid] == 0)) if n_obs > 0 else 0

        auc = np.nan

        if n_obs >= MIN_AUC_OBS and len(np.unique(col[valid])) > 1:
            auc = roc_auc_score(
                col[valid],
                preds[valid, j]
            )
...
def compute_context_coverage(y_true, context_mask_per_target):
    ...
    context_mask_per_target:
        [samples, target_antibiotic, context_antibiotic]
    """
...
class PerAntibioticPlattCalibrator:
    """
    Per-antibiotic Platt scaling:

        p_cal = sigmoid(a_j * logit_j + b_j)

    Important:
        If learned a_j <= 0, we disable calibration for that antibiotic,
        because a negative slope reverses rankings and can damage AUC.
    """
    ...
    def __init__(self, min_obs=30):
        ...
        self.was_disabled_negative_slope = None
    ...
    def fit(self, logits_val, y_val):
        ...
        self.was_disabled_negative_slope = np.zeros(num_items, dtype=bool)
        ...
        for j in range(num_items):
            ...
            try:
                ...
                if a <= 0:
                    ...
                    self.was_disabled_negative_slope[j] = True
                else:
                    self.a[j] = a
                    self.b[j] = b
    ...
    def to_dataframe(self, antibiotic_names=None):
        ...
        for j in range(len(self.a)):
            ...
            rows.append({
                ...
                "was_disabled_negative_slope": bool(
                    self.was_disabled_negative_slope[j]
                ),
            })
...
def train_stage2(
    ...
    num_items,
    stage1_test_probs,
    n_reliable_context_antibiotics
):
    ...
    print(
        f"Reliable context antibiotics: "
        f"{n_reliable_context_antibiotics}/{num_items}",
        flush=True
    )
    ...
    model = Stage2ContextPredictor(
        ...
        reliable_context_mask=reliable_context_mask,
    ...
    return {
        ...
        "delta_macro_stage2_minus_stage1": stage2_macro - stage1_macro,
        "delta_micro_stage2_minus_stage1": stage2_micro - stage1_micro,
        "context_coverage": coverage,
        "min_context_val_auc": MIN_CONTEXT_VAL_AUC,
        "n_reliable_context_antibiotics": n_reliable_context_antibiotics,
        "num_items": num_items,
        "reliable_context_fraction": (
            n_reliable_context_antibiotics / num_items
            if num_items > 0 else np.nan
        ),
        "calibration": "platt_per_antibiotic_positive_slope_only",
...
def train_species(species):
    ...
    for fold, (train_idx, test_idx) in enumerate(folds):
        ...
        raw_probs_val = sigmoid_np(logits_val)
        raw_probs_tst = sigmoid_np(logits_tst)

        raw_stage1_val_micro, raw_stage1_val_macro = compute_metrics(
            y_true=amr_val,
            preds=raw_probs_val
        )

        raw_stage1_test_micro, raw_stage1_test_macro = compute_metrics(
            y_true=amr_tst,
            preds=raw_probs_tst
        )

        print(
            f"Raw Stage 1 val | micro={raw_stage1_val_micro:.4f} | "
            f"macro={raw_stage1_val_macro:.4f}",
            flush=True
        )

        print(
            f"Raw Stage 1 test | micro={raw_stage1_test_micro:.4f} | "
            f"macro={raw_stage1_test_macro:.4f}",
            flush=True
        )

        # ====================================================
        # 3) AUC per antibiotic in validation
        # ====================================================
        df_val_auc = compute_antibiotic_auc_table(
            y_true=amr_val,
            preds=raw_probs_val,
        ...
        reliable_context_mask = (
            df_val_auc["auc"].fillna(-np.inf).values
            >= MIN_CONTEXT_VAL_AUC
        ).astype(np.float32)

        n_reliable = int(np.sum(reliable_context_mask))

        df_val_auc["is_reliable_context"] = reliable_context_mask.astype(bool)

        val_auc_path = os.path.join(
            OUTPUT_DIR,
            f"{safe_name(species)}_fold{fold}_stage1_val_auc_by_antibiotic.csv"
        )

        df_val_auc.to_csv(val_auc_path, index=False)

        print(
            f"Reliable context antibiotics with val AUC >= "
            f"{MIN_CONTEXT_VAL_AUC}: {n_reliable}/{num_items}",
            flush=True
        )

        if n_reliable == 0:
            print(
                "WARNING: No reliable context antibiotics for this species/fold. "
                "Stage 2 will receive empty context.",
                flush=True
            )
        ...
        print(
            ...
            f"disabled negative slopes: "
            f"{df_cal['was_disabled_negative_slope'].sum()}",
            flush=True
        )
        ...
        calibrated_stage1_val_micro, calibrated_stage1_val_macro = compute_metrics(
            y_true=amr_val,
            preds=context_probs_val
        )

        calibrated_stage1_test_micro, calibrated_stage1_test_macro = compute_metrics(
            y_true=amr_tst,
            preds=context_probs_tst
        )

        print(
            f"Calibrated Stage 1 val | "
            f"micro={calibrated_stage1_val_micro:.4f} | "
            f"macro={calibrated_stage1_val_macro:.4f}",
            flush=True
        )

        print(
            f"Calibrated Stage 1 test | "
            f"micro={calibrated_stage1_test_micro:.4f} | "
            f"macro={calibrated_stage1_test_macro:.4f}",
            flush=True
        )

        # ====================================================
        # 6) Train correlations
        # ====================================================
        ...
        for experiment in EXPERIMENTS:
            try:
                res = train_stage2(
                    ...
                    n_reliable_context_antibiotics=n_reliable
                )

                res["raw_stage1_val_auc_micro"] = raw_stage1_val_micro
                res["raw_stage1_val_auc_macro"] = raw_stage1_val_macro
                res["raw_stage1_test_auc_micro"] = raw_stage1_test_micro
                res["raw_stage1_test_auc_macro"] = raw_stage1_test_macro
                res["calibrated_stage1_val_auc_micro"] = calibrated_stage1_val_micro
                res["calibrated_stage1_val_auc_macro"] = calibrated_stage1_val_macro
                res["calibrated_stage1_test_auc_micro"] = calibrated_stage1_test_micro
                res["calibrated_stage1_test_auc_macro"] = calibrated_stage1_test_macro
...
for sp in species_list:
    try:
        ...
        partial_path = os.path.join(
            OUTPUT_DIR,
            "all_auc_filtered_context_partial_results.csv"
        )
...
raw_path = os.path.join(
    OUTPUT_DIR,
    "auc_filtered_context_raw_fold_results.csv"
)
...
if len(df_raw) > 0:
    summary_species = (
        ...
        .agg(
            ...
            delta_macro_stage2_minus_stage1=("delta_macro_stage2_minus_stage1", "mean"),
            delta_micro_stage2_minus_stage1=("delta_micro_stage2_minus_stage1", "mean"),
            context_coverage=("context_coverage", "mean"),
            n_reliable_context_antibiotics=("n_reliable_context_antibiotics", "mean"),
            reliable_context_fraction=("reliable_context_fraction", "mean"),
            raw_stage1_test_auc_micro=("raw_stage1_test_auc_micro", "mean"),
            raw_stage1_test_auc_macro=("raw_stage1_test_auc_macro", "mean"),
            calibrated_stage1_test_auc_micro=("calibrated_stage1_test_auc_micro", "mean"),
            calibrated_stage1_test_auc_macro=("calibrated_stage1_test_auc_macro", "mean"),
        )
    )
    ...
    summary_species_path = os.path.join(
        OUTPUT_DIR,
        "auc_filtered_context_summary_by_species.csv"
    )
    ...
    summary_global = (
        ...
        .agg(
            ...
            mean_delta_macro=("delta_macro_stage2_minus_stage1", "mean"),
            mean_delta_micro=("delta_micro_stage2_minus_stage1", "mean"),
            mean_context_coverage=("context_coverage", "mean"),
            mean_n_reliable_context_antibiotics=("n_reliable_context_antibiotics", "mean"),
            mean_reliable_context_fraction=("reliable_context_fraction", "mean"),
        )
    )
    ...
    summary_global_path = os.path.join(
        OUTPUT_DIR,
        "auc_filtered_context_global_summary.csv"
    )
    ...
    # ========================================================
    # BEST EXPERIMENT PER SPECIES
    # ========================================================
    best_per_species = (
        summary_species
        .sort_values(
            by=["species", "stage2_auc_macro"],
            ascending=[True, False]
        )
    ...
    best_path = os.path.join(
        OUTPUT_DIR,
        "auc_filtered_context_best_experiment_per_species.csv"
    )

    best_per_species.to_csv(best_path, index=False)

    print("\nBEST EXPERIMENT PER SPECIES:")
    print(best_per_species)
    print(f"\nSaved best experiment per species: {best_path}")
