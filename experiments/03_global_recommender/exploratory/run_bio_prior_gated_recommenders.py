...
# gamma = sigmoid(gamma_raw). Initialized around 0.10.
INITIAL_GAMMA = 0.10
...
class BioPriorGatedRecommender(pl.LightningModule):
    ...
    In both:
        ...
        final_logit = maldi_logit + gamma * prior_logit
    ...
    def __init__(
        ...
        # gamma = sigmoid(gamma_raw), initialized near initial_gamma
    ...
    def forward(
        ...
        final_logit = maldi_logit + gamma * prior_logit.view(-1, 1).float()

        return final_logit, delta, gate, gamma
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("gamma", gamma.detach(), prog_bar=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("gamma_val", gamma.detach(), prog_bar=False)
...
for model_mode in MODEL_MODES:
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(sample_indices)):
        ...
        print("Initial gamma:", float(torch.sigmoid(model.gamma_raw).detach().cpu()), flush=True)
        ...
        final_gamma = float(torch.sigmoid(model.gamma_raw).detach().cpu())

        print("Final gamma:", final_gamma, flush=True)
