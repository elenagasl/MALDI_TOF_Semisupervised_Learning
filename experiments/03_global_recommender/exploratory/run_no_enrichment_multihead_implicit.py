...
#
#   2) implicit_species_no_enrichment
#      - Global MALDI encoder
#      - Base recommender logit from MALDI + species + antibiotic
#
# Core idea:
#
#   z_maldi = MALDIEncoder(x)
#
#   base_logit = BaseMLP([
#       z_maldi,
#       species_emb,
...
MODEL_MODES = [
    ...
    "implicit_species_no_enrichment",
]
...
class NoEnrichmentRecommender(pl.LightningModule):
    """
    Unified model for the two no-enrichment baselines.

    1) species_multihead_no_enrichment:
        z_maldi = SpeciesHeadMALDIEncoder(MALDI, species_id)

    2) implicit_species_no_enrichment:
        z_maldi = GlobalMALDIEncoder(MALDI)

    In both cases:
        final_logit = MLP([z_maldi, species_emb, drug_emb])
    """
    ...
    def __init__(
        ...
        self.input_dim = maldi_emb_dim + species_emb_dim + drug_emb_dim

        self.recommender_mlp = self._make_mlp(
