"""
Species-Specific Recommender — Training Script (5-Fold Cross-Validation)
=========================================================================

Trains and evaluates a species-specific Neural Collaborative Filtering (NCF)
recommender for MALDI-TOF AMR prediction (Paper Objective 2).

Each instance of the model is trained on a single bacterial species.
The recommender treats each (spectrum, antibiotic) pair as an interaction
and predicts P(resistant) from the MALDI-TOF spectrum embedding, the
antibiotic embedding, and an AMR context vector encoding the known
resistance profile of the sample across all other antibiotics.

Model (AMRModel in models/amr_recommender.py)
----------------------------------------------
    dot-product interaction between:
    - spectrum encoder (MLP or Transformer)
    - antibiotic encoder (ECFP fingerprint MLP, GRU, CNN, Transformer, etc.)

Cross-validation
----------------
5-fold K-Fold over species-specific samples.
Prediction matrices are reconstructed after each fold and saved to CSV.

Output
------
Per-species result CSV with micro/macro AUC per fold and antibiotic.
"""
...
from models.amr_recommender import AMRModel
