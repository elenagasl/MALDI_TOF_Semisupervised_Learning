...
Re-evaluates species-specific MLP models under a cross-validation protocol
directly comparable to the global species-conditioned recommender, enabling
a fair apples-to-apples comparison (Paper Objective 1 vs Objective 3).

Models evaluated
----------------
1. Binary MLP
    - One independent model per (species, antibiotic) pair.
    - Trained on samples that have an observed label for that antibiotic.

2. Multilabel MLP
    - One model per species, one output logit per antibiotic.
    - Supports missing labels via masked BCE loss: each sample contributes
      gradient only for antibiotics where its label is observed.

Why no LPS model
-----------------
The LPS formulation requires complete rows (no missing antibiotic labels),
which forces exclusion of partially-labelled samples and makes it
incomparable to the recommender. Only binary and multilabel models are
evaluated here.

Data split
----------
Global K-Fold (default: 5) over all samples — identical fold indices to
those used when training the global recommender. Within each fold, a
separate model is trained and evaluated per species.

Metrics (per species and fold)
-------------------------------
- binary_micro_auc    : AUC over all observed (sample, antibiotic) pairs (binary models)
- binary_macro_auc    : Mean per-antibiotic AUC (binary models)
- multilabel_micro_auc: AUC over all observed pairs (multilabel model)
- multilabel_macro_auc: Mean per-antibiotic AUC (multilabel model)

Output files
------------
- fair_mlp_all_species_fold_results.csv        : Per-(species, fold) metrics
- fair_mlp_species_summary.csv                 : Summary statistics averaged across folds
- fair_mlp_species_summary_public_table.csv    : Publication-ready table
- fair_mlp_species_antibiotic_fold_results.csv : Per-(species, antibiotic, fold) results
- fair_mlp_global_fold_results.csv             : Globally aggregated metrics per fold
- fair_mlp_global_summary.json                 : Run metadata and global summary stats
- fair_mlp_run.log                             : Full training log

Example usage
-------------
# With previously tuned Optuna hyperparameters:
nohup python eval_fair_mlp.py \\
  --pickle-path "/data/COMBINED_MARISMA_DRIAMS_samples.pkl" \\
  --benchmark-roots "/results/benchmark_mlp_run1" "/results/benchmark_mlp_run2" \\
  --output-dir "/results/fair_mlp_evaluation" \\
  --device auto > fair_mlp.log 2>&1 &

# With default hyperparameters (no prior Optuna search needed):
nohup python eval_fair_mlp.py \\
  --pickle-path "/data/COMBINED_MARISMA_DRIAMS_samples.pkl" \\
  --output-dir "/results/fair_mlp_default_params" \\
  --device auto > fair_mlp.log 2>&1 &
"""
...
def load_pickle_payload(pickle_path: str) -> Dict[str, Any]:
    ...
    if missing:
        raise KeyError(f"Pickle is missing required keys: {missing}")
...
def clean_spectra_and_labels(
    ...
    """
    Removes only samples with invalid (non-finite) spectra.

    Importante:
        NaN values in AMR labels are handled via masking — samples are NOT dropped.
        NaN values in AMR labels are handled downstream via masking.
    """
...
def sanitize_params(params: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Normalises Optuna hyperparameter dicts to the expected format.
...
def discover_params_for_species(
    ...
    """
    Discovers Optuna hyperparameter dicts from previous benchmark runs.
    ...
    Falls back to DEFAULT_PARAMS when no prior hyperparameters are found.
    """
    ...
    for root in benchmark_roots:
        ...
        if not root_path.exists():
            logging.warning("Benchmark root not found and will be skipped: %s", root_path)
            continue
    ...
    # Lexicographic ordering gives priority to the most recent benchmark root
    ...
    for species_dir in candidate_species_dirs:
        ...
        if ml_path.exists():
            try:
                ...
                logging.warning("[%s] Could not load multilabel params from %s: %s", species, ml_path, e)
            ...
            for ab in antibiotics:
                ...
                if bin_path.exists():
                    try:
                        ...
                        logging.warning(
                            "[%s | %s] Could not load binary params from %s: %s",
                            species,
                            ab_str,
                            bin_path,
                            e,
...
def activation_from_name(name: str) -> nn.Module:
    ...
    logging.warning("Unrecognised activation '%s'. Defaulting to GELU.", name)
    return nn.GELU()
...
def build_optimizer(model: nn.Module, params: Dict[str, Any]) -> torch.optim.Optimizer:
    ...
    logging.warning("Unrecognised solver '%s'. Defaulting to Adam.", solver)
    return torch.optim.Adam(model.parameters(), lr=lr)
...
def compute_micro_macro_auc_matrix(
    ...
    macro:
        mean per-antibiotic AUC, ignoring antibiotics that cannot be evaluated
    """
...
def select_valid_antibiotics_for_species_fold(
    ...
    """
    Selects antibiotics that can be evaluated for a given species and fold.

    Criteria:
        - minimum number of observed training samples
        - both classes (resistant and susceptible) present in training
        - minimum number of observed validation samples
...
def evaluate_species_fold(
    ...
    """
    Returns:
        - per-species fold record
        - per-(species, antibiotic) fold records
        - y_val_binary, p_val_binary
        - y_val_multilabel, p_val_multilabel

    Returned matrices have shape (n_val_sp, n_antibiotics_total)
    with NaN where no label or prediction is available.
    """
    ...
    # --------------------------------------------------------
    # --- 1) Binary MLP — one model per antibiotic ---
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # --- 2) Multilabel MLP — one model per species with masked BCE ---
    # --------------------------------------------------------
    ...
    # Compute per-antibiotic multilabel AUC and populate records
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Falls back to defaults for species/antibiotics without saved params."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Defaults to all species in the pickle (same as global recommender)."
        ),
    )
    ...
    parser.add_argument(
        ...
        help="Minimum observed samples per (species, antibiotic) in training.",
    )
    ...
    parser.add_argument(
        ...
        help="Minimum observed samples per (species, antibiotic) in validation.",
    )
...
def main() -> None:
    ...
    if len(X_all) != len(species_all) or len(X_all) != len(amr_all):
        raise ValueError("Inconsistent lengths between data, label, and amr arrays.")

    if len(antibiotics_all) != amr_all.shape[1]:
        raise ValueError(
            f"Antibiotic count mismatch: len(antibiotics)={len(antibiotics_all)} "
    ...
    if not unique_species:
        raise RuntimeError("No species selected — nothing to evaluate.")
    ...
    # Cache hyperparameters per species to avoid re-loading
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(sample_indices)):
        ...
        # Global prediction matrices for this fold (aggregated across species)
