"""
GPU-Accelerated PIKE Kernel for MALDI-TOF Spectra
===================================================

Implements the Peak Intensity Kernel Estimation (PIKE) for comparing two
MALDI-TOF mass spectra as a similarity function.

The PIKE kernel computes a symmetric similarity between two sets of
(m/z, intensity) peak pairs by evaluating a Gaussian radial basis
function over the pairwise m/z distances, weighted by peak intensities.
The normalised PIKE score is:

    PIKE(X, Y) = K(X, Y) / sqrt(K(X, X) * K(Y, Y))

where K(X, Y) = Σᵢ Σⱼ  iₓ · iᵧ · exp( -t · |mzₓᵢ - mzᵧⱼ|² )

This implementation runs entirely on GPU via PyTorch tensor operations,
avoiding peak-level for-loops for efficient batch computation.

Classes
-------
PIKE_GPU
    Callable that computes the PIKE kernel between two spectra (or a
    spectrum against itself). Returns the normalised similarity score.

Functions
---------
calculate_PIKE_gpu
    Batch wrapper for computing pairwise PIKE similarities across a
    dataset of preprocessed spectra.
"""
